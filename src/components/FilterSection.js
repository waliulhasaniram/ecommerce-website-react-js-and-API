...
const FilterSection=()=>{
    const {filters: {text, category, color, price, maxPrice, minPrice}, updateFilterValue, allProducts, clearFilter} = useFilterContext();

    // TO GET ALL UNIQUE DATA
    ...
    return <Wrapper>
          ...
          <div>
               {
                  categoryData.map((curElem, index)=>{
                    return <button key={index} name="category" type="category" value={curElem} onClick={updateFilterValue} 
                                   className={curElem === category ? "active" : ""}>{curElem}</button>
                  })
               }
          </div>
        </div>
        ...
        <div className="filter_price">
            <h3>Price</h3>
            <p>{Intl.NumberFormat("BDT" , {style:"currency", currency:"BDT", maximumFractionDigits:2}).format(price/100)}</p>
